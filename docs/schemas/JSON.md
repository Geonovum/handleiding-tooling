...
Om een bij het draaien van Imvertor ook een JSON-schem te maken, moeten 2 regels opgenomen worden in de [Imvertor-propertiesfile](https://armatiek.nl/doc/imvertor/imvertor-properties/1.0/) :
...
Er zijn ook organisatie specifieke instellingen die in een organisatiespecifieke configuratiefile verwerkt zijn. 
Als deze ook aangepast moeten worden, moet dat in overleg met de Geonovum coordinator voor Imvertor.

Imvertor kan niet overweg met 2 stereotypen op 1 objectklasse. Een tweede moet dan verwijderd worden. Dat kan eventueel toegepast worden door de 2 stereotypen alleen in het conceptuele model op te nemen, maar in het logische model slechts1.
...
Geonovum heeft ook nog enkele wensen uitstaan bij Armatiek: [#585](https://github.com/Imvertor/Imvertor-Maven/issues/585) t/m [#590](https://github.com/Imvertor/Imvertor-Maven/issues/590).
...
- Rrelaties die twee kanten opgaan kunnen leiden tot eeuwige loop bij het valideren of bij het aanmaken van voorbeeldbestanden. Een optie is het weglaten van één of beide reichtingen.
...
- Realiseer je dat het schema bedoelt is voor implementatie en daarom kan die afwijken van het model. Een voorbeeld van deze afwijking is het attribuut "entityType". Dit attribuut komt niet voor in het UML.
- Voor hetzelfde UML-model kunnen verschillende schema's bestaan met ieder verschillende doelen. bv voor input of output. Het atribuut "tijdstipRegistratie" komt bv in in de input voor, maar wel in de output.
...
Het loont om voorbeeldbestanden te maken. Dat geest inzicht in de bruikbaarheid van het schema.
...
Als ze er nog niet zijn biedt Oxygen de mogelijkheid om voorbeeldbestanden met dummy waarden aan te maken. 
